...
public static class RangeSyntax
{
    ...
    public static RangeSyntax<WorkItemCost>? ParseCost(string text)
    {
        return RangeSyntax<WorkItemCost>.Parse(text, TryParseCost);

        static bool TryParseCost(string text, out WorkItemCost result)
        {
            return Enum.TryParse(text, ignoreCase: true, out result);
        }
    }

    public static RangeSyntax<WorkItemVersion>? ParseVersion(string text)
    {
        return RangeSyntax<WorkItemVersion>.Parse(text, WorkItemVersion.TryParse);
    }
}
